 ...
 */
$(function () {
	var parentwin = window.opener, hash = location.hash.substring(1), datas = hash.split('&'), item = null, args = {}, 
		params = null, results = null,htmls = "", search = $('.search > ul'), host = location.host;
	// 解析url 参数
	for (var i = 0, t; t = datas[i]; i++) {
		item = t.split('=');
		args[item[0]] = item[1];
	}
	for(var i in parentwin.excerpts) {
		console.log(i + "=========" + parentwin.excerpts[i]);
	}
	// 日期
	if(args['type'] === 'date') {
		...
		params = parentwin.excerpts;
		results = params[args['q']];
		console.log(results);
	}
